  ...
  methods: {
    getBadgeClasses() {
      const classes = this.outline
        ? `border ${this.getOutlineClasses()} border-body-color text-body-color`
        : `bg-body-color ${this.getBackgroundClasses()} text-white`

      return this.bgOpacity
        ? `${classes} bg-body-color/10 dark:bg-gray/10 !text-dark dark:!text-gray`
        : classes
    },
    getOutlineClasses() {
      return this.getCommonClasses() + (this.bgOpacity ? ' bg-body-color/10 dark:bg-gray/10' : '')
    },
