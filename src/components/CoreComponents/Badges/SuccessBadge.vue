  ...
  methods: {
    getBadgeClasses() {
      const classes = this.outline
        ? `border ${this.getOutlineClasses()} border-green-dark text-green-dark`
        : `bg-green-dark ${this.getBackgroundClasses()} text-white`

      return this.bgOpacity ? `${classes} !bg-green-dark/10 !text-green-dark` : classes
    },
    getOutlineClasses() {
      return this.getCommonClasses() + (this.bgOpacity ? ' bg-green-dark/10' : '')
    },
