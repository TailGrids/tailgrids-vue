          ...
          <div class="py-8 text-center">
            <div class="relative inline-block mb-8 text-left">
              <button
                ...
                class="bg-secondary flex items-center rounded-[5px] px-5 py-[13px] text-base font-medium text-white"
              >
                Dropdown Button
                <span class="pl-4">
                  <svg
              ...
              >
                <template v-for="(item, index) in dropdownItems" :key="index">
                  <a
                    v-if="item.href"
                    :href="item.href"
                    class="text-body-color dark:text-dark-6 hover:bg-secondary/5 hover:text-secondary block px-5 py-2 text-base"
                  >
                    {{ item.text }}
                  </a>
                </template>
