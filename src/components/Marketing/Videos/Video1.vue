...
const useClickOutside = (handler: () => void) => {
  const maybeHandler = (event: MouseEvent) => {
    if (videoContainer.value && !videoContainer.value.contains(event.target as Node)) {
      handler()
    }
  }

  onMounted(() => {
    window.addEventListener('mousedown', maybeHandler)
  })

  onUnmounted(() => {
    window.removeEventListener('mousedown', maybeHandler)
  })
}

useClickOutside(closeVideo)
</script>

<template>
  <section class="bg-white py-20 dark:bg-dark lg:py-[120px]">
    <div ref="videoContainer" class="container">
      <div class="-mx-4 flex flex-wrap justify-center">
        <div class="w-full px-4 lg:w-10/12">
          <div class="relative z-20 h-[300px] overflow-hidden rounded-lg md:h-[450px]">
            ...
            <div
              class="absolute left-0 top-0 z-10 flex h-full w-full items-center justify-center bg-primary bg-opacity-90"
            >
              <a
                href="/#"
                @click="openVideo"
                class="absolute z-20 flex h-20 w-20 items-center justify-center rounded-full bg-white text-primary dark:bg-dark-2 dark:text-white md:h-[100px] md:w-[100px]"
              >
                <span
                  class="absolute right-0 top-0 z-[-1] h-full w-full animate-ping rounded-full bg-white bg-opacity-20 delay-300 duration-1000"
                ></span>
