...
const onSwiper = (swiperInstance: SwiperType) => {
  // Save the Swiper instance to the ref variable
  sliderRef.value = swiperInstance

  // Initialize Swiper after the instance is available
  initializeSwiper()
}

const initializeSwiper = () => {
  if (sliderRef.value) {
    // Call Swiper methods only after the instance is fully initialized
    sliderRef.value.slideTo(0) // Adjust the initial slide index as needed
  }
}

onMounted(() => {
  // You can also initialize Swiper here if needed
  initializeSwiper()
})
